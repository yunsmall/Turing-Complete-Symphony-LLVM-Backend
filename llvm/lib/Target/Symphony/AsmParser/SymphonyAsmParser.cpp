//===- SymphonyAsmParser.cpp - Parse Symphony assembly to MCInst instructions -===//
//
...
#include "MCTargetDesc/SymphonyTargetStreamer.h"
#include "TargetInfo/SymphonyTargetInfo.h"
...
#define DEBUG_TYPE "symphony-asm-parser"

struct SymphonyOperand;

class SymphonyAsmParser : public MCTargetAsmParser {
  const MCRegisterInfo &MRI;

  // enum SymphonyRegisterType { Symphony_Generic, Symphony_SR, Symphony_UR };
  SMLoc getLoc() const { return getParser().getTok().getLoc(); }

  SymphonyTargetStreamer &getTargetStreamer() {
    MCTargetStreamer &TS = *getParser().getStreamer().getTargetStreamer();
    return static_cast<SymphonyTargetStreamer &>(TS);
  }
...
#undef GET_OPERAND_DIAGNOSTIC_TYPES
  ...
  SymphonyAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
  ...
  //   return getSTI().getFeatureBits()[Symphony::FeatureWindowed];
  // };
};
...
struct SymphonyOperand : public MCParsedAsmOperand {
  ...
  SymphonyOperand(KindTy K) : MCParsedAsmOperand(), Kind(K) {}

public:
  SymphonyOperand(const SymphonyOperand &o) : MCParsedAsmOperand() {
    Kind = o.Kind;
    StartLoc = o.StartLoc;
    EndLoc = o.EndLoc;
    switch (Kind) {
  ...
  static std::unique_ptr<SymphonyOperand> createToken(StringRef Str, SMLoc S) {
    auto Op = std::make_unique<SymphonyOperand>(Token);
    Op->Tok = Str;
    Op->StartLoc = S;
    Op->EndLoc = S;
    return Op;
  ...
  static std::unique_ptr<SymphonyOperand> createReg(unsigned RegNo, SMLoc S,
                                                  SMLoc E) {
    auto Op = std::make_unique<SymphonyOperand>(Register);
    Op->Reg.RegNum = RegNo;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
  ...
  static std::unique_ptr<SymphonyOperand> createImm(const MCExpr *Val, SMLoc S,
                                                  SMLoc E) {
    auto Op = std::make_unique<SymphonyOperand>(Immediate);
    Op->Imm.Val = Val;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
...
unsigned SymphonyAsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
                                                     unsigned Kind) {
...
bool SymphonyAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
                                         MCStreamer &Out,
                                         const MCSubtargetInfo *STI) {
  // Inst.setLoc(IDLoc);
  ...
  // }
  // case Symphony::MOVI: {
  //   SymphonyTargetStreamer &TS = this->getTargetStreamer();
  //
  //   // Expand MOVI operand
  ...
  //       TmpInst.setOpcode(Symphony::L32R);
...
bool SymphonyAsmParser::matchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((SymphonyOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IDLoc;
    }
...
ParseStatus SymphonyAsmParser::parsePCRelTarget(OperandVector &Operands) {
  ...
  Operands.push_back(SymphonyOperand::createImm(Expr, S, getLexer().getLoc()));
  return ParseStatus::Success;
}

bool SymphonyAsmParser::parseRegister(MCRegister &Reg, SMLoc &StartLoc,
                                    SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  Reg = Symphony::NoRegister;
...
ParseStatus SymphonyAsmParser::parseRegister(OperandVector &Operands,
                                           bool AllowParens) {
  SMLoc FirstS = getLoc();
  ...
  case AsmToken::Integer:
    ...
    // if (RegType == Symphony_UR) {
    //   int64_t RegCode = getLexer().getTok().getIntVal();
    //   RegNo = Symphony::getUserRegister(RegCode, MRI);
    // } else {
  ...
  // if (!Symphony::checkRegister(RegNo, getSTI().getFeatureBits(), RAType))
  //   return ParseStatus::NoMatch;

  if (HadParens)
    Operands.push_back(SymphonyOperand::createToken("(", FirstS));
  SMLoc S = getLoc();
  SMLoc E = getParser().getTok().getEndLoc();
  getLexer().Lex();
  Operands.push_back(SymphonyOperand::createReg(RegNo, S, E));

  if (HadParens) {
    getParser().Lex(); // Eat ')'
    Operands.push_back(SymphonyOperand::createToken(")", getLoc()));
  }

  return ParseStatus::Success;
}
ParseStatus SymphonyAsmParser::parseMem(OperandVector &Operands) {
  SMLoc StartLoc = getLexer().getLoc();

  // 检查当前token是否是'['，如果不是则返回NoMatch
  ...
  // 吃掉'[' token
  getLexer().Lex();
  Operands.push_back(SymphonyOperand::createToken("[", StartLoc));

  // 解析内存操作数内部的内容
  SMLoc ContentStart = getLexer().getLoc();
  SMLoc ContentEnd;
  ...
  if (parseRegister(Operands).isSuccess()) {
    ...
    if (getLexer().isNot(AsmToken::RBrac)) {
      return Error(getLexer().getLoc(), "expected ']' after memory operand");
    }
    SMLoc EndLoc = getLexer().getLoc();
    getLexer().Lex(); // 吃掉']'
    Operands.push_back(SymphonyOperand::createToken("]", EndLoc));
    // Operands.push_back(SymphonyOperand::createReg(RegNo, StartLoc, EndLoc));
    return ParseStatus::Success;
  }

  // 不是寄存器，尝试解析立即数
  const MCExpr *ImmValue;
  if (!getParser().parseExpression(ImmValue, ContentEnd)) {
    // 成功解析立即数，创建立即数内存操作数
    ...
    Operands.push_back(SymphonyOperand::createImm(ImmValue, StartLoc, EndLoc));
    Operands.push_back(SymphonyOperand::createToken("]", EndLoc));
    return ParseStatus::Success;
  }

  // 既不是寄存器也不是立即数，解析失败
  // return Error(ContentStart, "expected register or immediate expression in memory operand");
  return ParseStatus::Failure;
}
ParseStatus SymphonyAsmParser::parseImmediate(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E;
  const MCExpr *Res;
  ...
  Operands.push_back(SymphonyOperand::createImm(Res, S, E));
  return ParseStatus::Success;
}

ParseStatus SymphonyAsmParser::parseOperandWithModifier(OperandVector &Operands) {
  return ParseStatus::Failure;
}
...
bool SymphonyAsmParser::parseOperand(OperandVector &Operands, StringRef Mnemonic) {
  ...
  if (parseMem(Operands).isSuccess())
    return false;
...
// bool SymphonyAsmParser::ParseInstructionWithSR(ParseInstructionInfo &Info,
//                                              StringRef Name, SMLoc NameLoc,
//                                              OperandVector &Operands) {
//   Symphony::RegisterAccessType RAType =
//       Name[0] == 'w' ? Symphony::REGISTER_WRITE
//                      : (Name[0] == 'r' ? Symphony::REGISTER_READ
//                                        : Symphony::REGISTER_EXCHANGE);
//
...
//     Operands.push_back(SymphonyOperand::createToken(Name.take_front(3), NameLoc));
//
...
//   } else {
//     // First operand is token for instruction
//     Operands.push_back(SymphonyOperand::createToken(Name, NameLoc));
//
//     // Parse first operand
//     if (parseOperand(Operands, Name))
//       return true;
...
//     if (parseOperand(Operands, Name, Name[1] == 's' ? Symphony_SR : Symphony_UR,
//                      RAType))
//       return true;
//   }
//
...
bool SymphonyAsmParser::parseInstruction(ParseInstructionInfo &Info,
                                       StringRef Name, SMLoc NameLoc,
                                       OperandVector &Operands) {
...
// bool SymphonyAsmParser::parseLiteralDirective(SMLoc L) {
//   MCAsmParser &Parser = getParser();
//   const MCExpr *Value;
...
ParseStatus SymphonyAsmParser::parseDirective(AsmToken DirectiveID) {
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeSymphonyAsmParser() {
  RegisterMCAsmParser<SymphonyAsmParser> X(getTheSymphonyTarget());
}
